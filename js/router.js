...
let _homeOnlyEls = document.querySelectorAll('.home-only');
...
function _showPage(name) {
  ...
  _homeOnlyEls.forEach(el => el.classList.toggle('hidden', name !== HOME_PAGE));
}
...
const _titleMap = { home: '实录：跨越11512年的人类文明历史年表 shilu.org', events: '全站事件列表 - 实录 shilu.org', map: '历史地图 - 实录 shilu.org', detail: '事件详情 - 实录 shilu.org', quiz: '历史趣味问答 - 实录 shilu.org' };
const _descMap = { home: '跨越11512年的交互式地球文明年表，覆盖公元前9600年至公元1912年的重大历史事件。按年份浏览政治、军事、文化、科技等领域的重要时刻，配合地图直观呈现。', events: '浏览实录全站所有历史事件，按分类筛选，搜索任意年份、地区、关键词。', map: '在地图上查看全球历史事件的时空分布，从安纳托利亚到印度河流域，纵览人类文明足迹。', quiz: '通过趣味问答测试历史知识，包含朝代、年份、事件等多种题型。逐步提升修为等级。' };
const _canonicalMap = { home: 'https://shilu.org/', events: 'https://shilu.org/?page=events', map: 'https://shilu.org/?page=map', quiz: 'https://shilu.org/?page=quiz' };

function _updatePageMeta(page) {
  const title = _titleMap[page] || _titleMap.home;
  const desc = _descMap[page] || _descMap.home;
    ...
    html += `<div class="search-result-item" data-year="${evt.y}" data-title="${_escHtml(evt.t)}">
      ...
      <div class="search-result-info">
        ...
        <div class="search-result-meta">
          <span class="search-result-tag">${_escHtml(evt.c || '事件')}</span>
