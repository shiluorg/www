...
function _dynastyName(d) {
  const lang = getCurrentLang();
  return lang === 'en' ? (d.nameEn || d.name) : d.name;
}

function _gi(year) { return state.contentYearIndex[year]; }
function _nearestGi(year) {
  const idx = state.contentYearIndex[year];
  if (idx !== undefined) return idx;
  return HashSearch.findNearestIndex(year, state.contentYears);
}
function _ci2x(i, w) { const t = state.contentYears.length; return t < 2 ? 0 : (i / (t - 1)) * w; }
function _x2ci(x, w) { const cy = state.contentYears, t = cy.length; if (t < 2) return 0; return Math.max(0, Math.min(t - 1, Math.round((x / w) * (t - 1)))); }
...
function _dynastyWeights() {
  const cy = state.contentYears;
  const lang = getCurrentLang();
  const ver = cy.length + '|' + (cy[0] || '') + '|' + (cy[cy.length-1] || '') + '|' + lang;
  if (_dynastyWeightsCache && _dynastyWeightsVer === ver) return _dynastyWeightsCache;
  const eras = lang === 'en' ? HashSearch.worldEras : HashSearch.dynasties;
  const result = [...eras].sort((a, b) => a.start - b.start).map(d => {
    let c = 0;
...
function _dynastyDrawNow() {
  ...
  for (const { d, width } of layout) {
    ...
    const isA = state.selectedDynasty === d.id, isC = state.currentYear >= d.start && state.currentYear <= d.end, isH = _state.hoverDynasty === d.id;
    ...
    if (width > 14) {
      ...
      const label = _dynastyName(d);
      ctx.fillText(label, x1 + width / 2, by + bh / 2);
    }
    xo = x2;
  }
  const ci = _gi(state.currentYear);
  if (ci !== undefined && state.contentYears.length > 1) {
...
function _positionTooltip(el, cx, cy, ox = 12, oy = -32, dw = 200, dh = 40) {
  const vw = window.innerWidth, vh = window.innerHeight;
  const tx = cx + ox, ty = cy + oy;
  el.style.left = Math.min(tx, vw - dw - 8) + 'px';
  el.style.top = Math.max(8, Math.min(ty, vh - dh - 8)) + 'px';
}
...
function _dynastyHandleMove(e) {
  ...
  if (d) {
    ...
    _state.tooltipD.textContent = dict.dynastyTooltip(_dynastyName(d), d.start, d.end);
    _state.tooltipD.classList.remove('hidden');
    _positionTooltip(_state.tooltipD, e.clientX, e.clientY - 32, 12, -32);
...
function _getCalendarTicks(drawW) {
  ...
  for (let i = 0; i < total; i++) {
    const y = years[i];
    let type;
    if (y % 100 === 0) type = 'major';
    else if (y % 10 === 0) type = 'mid';
    else type = 'minor';
    ticks.push({ x: _ci2x(i, drawW), year: y, type, index: i });
  }
  _calendarTickCache = ticks;
  _calendarTickCacheKey = key;
  return ticks;
}
...
function _calendarDrawNow() {
  ...
  ctx.clearRect(0, 0, w, h);
  const px = 8, drawW = _dw(w, px), tt = h * 0.1, th = h * 0.45, years = state.contentYears, total = years.length;
  if (total < 2) return;
  ...
  if (state.selectedDynasty) {
    const eras = getCurrentLang() === 'en' ? HashSearch.worldEras : HashSearch.dynasties;
    const d = eras.find(d => d.id === state.selectedDynasty);
    if (d) {
      ...
      if (si !== undefined && ei !== undefined) {
        ...
        ctx.fillStyle = `rgba(${r},${g},${b},0.12)`; ctx.fillRect(dx1, 0, dx2 - dx1, h);
        ctx.strokeStyle = `rgba(${r},${g},${b},0.5)`; ctx.lineWidth = 1; ctx.setLineDash([6, 3]);
  ...
  const ticks = _getCalendarTicks(drawW);
  for (const { x, year, type } of ticks) {
    const sx = x + px;
    if (sx < -20 || sx > w + 20) continue;
    const isC = year === state.currentYear, isH = year === _state.hoverYear;
    if (isC) { ctx.fillStyle = _cssVarRgba('--color-accent', 0.15); ctx.fillRect(sx - 2, 0, 4, h); }
    if (type === 'major') { ctx.strokeStyle = _cssVar('--color-timeline-major'); ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(sx, tt); ctx.lineTo(sx, tt + th * 1.44); ctx.stroke(); }
    else if (type === 'mid') { ctx.strokeStyle = _cssVar('--color-timeline-mid'); ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(sx, tt); ctx.lineTo(sx, tt + th); ctx.stroke(); }
    else { ctx.strokeStyle = isC || isH ? _cssVar('--color-timeline-major') : _cssVar('--color-timeline-minor'); ctx.lineWidth = isC || isH ? 1.5 : 0.5; ctx.beginPath(); ctx.moveTo(sx, tt); ctx.lineTo(sx, tt + th * 0.56); ctx.stroke(); }
    if (isC || isH) {
      ctx.fillStyle = isC ? _cssVar('--color-accent') : _cssVar('--color-dynasty');
      ctx.beginPath(); ctx.arc(sx, tt + h * 0.5, 4, 0, Math.PI * 2); ctx.fill();
...
function calendarYearAt(cx) {
  ...
  if (state.contentYears.length > 1) { const idx = _x2ci(rx - px, drawW); return state.contentYears[Math.max(0, Math.min(state.contentYears.length - 1, idx))]; }
  return 0;
}
...
function init() {
  ...
  C.addEventListener('mousemove', e => {
    ...
    if (y >= HashSearch.YEAR_MIN && y <= HashSearch.YEAR_MAX) {
      _state.hoverYear = y; _state.tooltipC.textContent = HashSearch.formatYear(y); _state.tooltipC.classList.remove('hidden');
      _positionTooltip(_state.tooltipC, e.clientX, e.clientY - 28, 12, -28, 120, 30);
      calendarDraw();
    } else { _state.hoverYear = null; _state.tooltipC.classList.add('hidden'); calendarDraw(); }
  });
